...
interface JobsFilterProps {
  countriesList: string[]; // Updated type to match the current data structure
}
...
const JobsFilter = ({ countriesList }: JobsFilterProps) => {
  ...
  // Ensure countriesList is an array before mapping
  const isCountriesListArray = Array.isArray(countriesList);
    ...
    <div className="relative mt-11 flex w-full justify-between gap-5 max-sm:flex-col sm:items-center">
      ...
      <Select onValueChange={(value) => handleUpdateParams(value)}>
        ...
        <SelectContent className="body-semibold max-h-[350px] max-w-[250px]">
          <SelectGroup>
            {isCountriesListArray ? (
              countriesList.map((country: string) => (
                <SelectItem key={country} value={country} className="px-4 py-3">
                  {country}
                </SelectItem>
              ))
            ) : (
